...
class _YoYoPlayerState extends State<YoYoPlayer> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final List<Widget> videoChildren = <Widget>[
      GestureDetector(
        onTap: () {
          toggleControls();
        },
  ...
  void urlCheck(String url) {
    final RegExp netRegex = new RegExp(RegexResponse.regexHTTP);
    final bool isNetwork = netRegex.hasMatch(url);
      ...
      (RegExpMatch regExpMatch) async {
        ...
        audioMatches.forEach(
          (RegExpMatch regExpMatch2) async {
            ...
            final bool isNetwork = netRegex.hasMatch(audioURL);
            final RegExpMatch? match = netRegex2.firstMatch(video);
    ...
    if (offline == false) {
      ...
      if (playType == "MP4") {
        ...
        controller = VideoPlayerController.network(url!, formatHint: VideoFormat.hls)
          ..initialize().then((_) => setState(() => hasInitError = false)).catchError((e) => setState(() => hasInitError = true));
      }
    } else {
      print("--- Player Status ---\nplay url : $url\noffline : $offline\n--- start playing –––");
      controller = VideoPlayerController.file(File(url!))
        ..initialize().then((value) => setState(() => hasInitError = false)).catchError((e) => setState(() => hasInitError = true));
    }
  }
